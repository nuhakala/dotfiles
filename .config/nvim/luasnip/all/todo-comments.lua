--[[
--  This file contains some copied code from luasnips to create nice comments
--  https://github.com/L3MON4D3/LuaSnip/wiki/Cool-Snippets#all---todo-commentsnvim-snippets
--  I slightly modified todo_snippet_nodes function to ensure that the order of
--  formats is always the same. I also switched the order in which the inputs rotate
--  when snippet gets activated. I changed the content section to be first, because
--  that is what I am interested to modify, not the header or signature.
--]]
...
_G.luasnip.vars = {
  username = 'nuhakala',
  email = 'nuhakala@mailbox.org',
  github = 'https://github.com/nuhakala',
}
...
local todo_snippet_nodes = function(aliases, opts)
  ...
  -- Insert manually, to ensure order
  table.insert(sigmark_nodes, marks.empty())
  table.insert(sigmark_nodes, marks.date())
  table.insert(sigmark_nodes, marks.date_signature())
  table.insert(sigmark_nodes, marks.date_signature_with_email())
  table.insert(sigmark_nodes, marks.signature())
  table.insert(sigmark_nodes, marks.signature_with_email())
  ...
  local comment_node = fmta('<> <>: <> <> <><>', {
    ...
    c(3, aliases_nodes), -- [name-of-comment]
    i(1), -- {comment-text}
